...
const formatTunisianNumber = (val: string) => {
  ...
  const localDigits = rawDigits.startsWith("216") ? rawDigits.slice(3) : rawDigits;
  ...
  if (limited.length <= 5) return result + limited.slice(0, 2) + " " + limited.slice(2);
  return result + limited.slice(0, 2) + " " + limited.slice(2, 5) + " " + limited.slice(5);
};
...
export default function ProfileEdit() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!tunisiaRegex.test(form.mobileNumber)) {
      setPhoneError("Mobile number must be exactly 8 digits after +216 (e.g., +216 58 496 156)");
      return;
    }
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] flex flex-col items-center justify-center px-6 py-12 relative">
      ...
      {isPasswordModalOpen && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border w-full max-w-md p-6 relative">
            <button onClick={() => setIsPasswordModalOpen(false)} className="absolute top-4 right-4 text-muted-foreground hover:text-foreground">
              <X size={20} />
            </button>
            <div className="flex items-center gap-2 mb-6">
              <Lock size={20} className="text-accent" />
              <h2 className="font-['Barlow_Condensed'] font-extrabold text-2xl uppercase tracking-widest text-foreground">Change Password</h2>
            </div>
            
            {pwdError && <div className="mb-4 p-3 border border-red-500/30 bg-red-500/10 text-red-500 font-['JetBrains_Mono'] text-xs">{pwdError}</div>}
            {pwdSuccess && <div className="mb-4 p-3 border border-emerald-500/30 bg-emerald-500/10 text-emerald-500 font-['JetBrains_Mono'] text-xs">Password changed successfully!</div>}
            
            <form onSubmit={handlePasswordChange} className="space-y-4">
              <div className="space-y-1">
                <label className="font-['Barlow_Condensed'] font-bold uppercase tracking-widest text-xs text-foreground">Old Password</label>
                <Input type="password" required value={pwdForm.old} onChange={e => setPwdForm({...pwdForm, old: e.target.value})} className="font-['JetBrains_Mono'] bg-secondary/50 border-border" />
              </div>
              <div className="space-y-1">
                <label className="font-['Barlow_Condensed'] font-bold uppercase tracking-widest text-xs text-foreground">New Password</label>
                <Input type="password" required value={pwdForm.new} onChange={e => setPwdForm({...pwdForm, new: e.target.value})} className="font-['JetBrains_Mono'] bg-secondary/50 border-border" />
                {pwdForm.new && (
                  <div className="mt-2">
              ...
              <div className="space-y-1">
                <label className="font-['Barlow_Condensed'] font-bold uppercase tracking-widest text-xs text-foreground">Repeat New Password</label>
                <Input type="password" required value={pwdForm.repeat} onChange={e => setPwdForm({...pwdForm, repeat: e.target.value})} className="font-['JetBrains_Mono'] bg-secondary/50 border-border" />
              </div>
              <Button type="submit" disabled={isChangingPwd} className="w-full font-['Barlow_Condensed'] font-bold uppercase tracking-widest text-sm mt-4" size="lg">
      ...
      <div className="w-full max-w-lg mb-4 flex justify-start">
        <Button variant="ghost" onClick={() => navigate("/settings")} className="font-['Barlow_Condensed'] uppercase tracking-widest font-bold text-muted-foreground">
      ...
      <div className="w-full max-w-lg border border-border bg-card p-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Button variant="outline" size="sm" type="button" onClick={() => setIsPasswordModalOpen(true)} className="font-['Barlow_Condensed'] uppercase tracking-widest">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="flex items-center gap-6 mb-8 p-4 bg-secondary/30 border border-border">
            <Avatar user={{ ...user, avatarUrl: form.avatarUrl }} size="xl" className="shadow-xl shrink-0" />
          ...
          <div className="space-y-2">
            ...
            />
            {phoneError && (
              <p className="font-['Barlow'] text-xs text-red-500 mt-1">{phoneError}</p>
            )}
          </div>
