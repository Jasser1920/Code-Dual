...
import { Zap, Trophy, Code2, TrendingUp, Calendar, ArrowRight } from "lucide-react";
import { LEADERBOARD, PROBLEMS, DIFF_BG } from "../data/mock";
...
export default function Profile() {
  ...
  const player = LEADERBOARD.find((p) => p.name === username) ?? LEADERBOARD[0];
      ...
      <div className="grid lg:grid-cols-3 gap-8 mb-10">
        <div className="lg:col-span-2 flex items-start gap-6">
          <div className="w-16 h-16 bg-secondary border border-border flex items-center justify-center shrink-0">
            <span className="font-['Barlow_Condensed'] font-extrabold text-2xl text-muted-foreground">{player.name[0].toUpperCase()}</span>
          </div>
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="font-['JetBrains_Mono'] font-bold text-2xl text-foreground">{player.name}</h1>
              <span className="font-['JetBrains_Mono'] text-xs text-muted-foreground border border-border px-2 py-0.5">{player.country}</span>
              {player.streak > 5 && (
                <div className="flex items-center gap-1 bg-accent/10 border border-accent/30 px-2 py-0.5">
                  <Zap size={11} className="text-accent" />
                  <span className="font-['JetBrains_Mono'] text-xs text-accent">{player.streak} streak</span>
                </div>
              )}
            </div>
            <div className="flex items-center gap-6 mt-3">
              <div>
                <div className="font-['Barlow_Condensed'] font-extrabold text-3xl text-foreground">{player.rating}</div>
                <div className="font-['Barlow'] text-xs text-muted-foreground">ELO Rating</div>
              </div>
              <div>
                <div className="font-['Barlow_Condensed'] font-extrabold text-3xl text-foreground">#{player.rank}</div>
        ...
        <div className="flex items-start justify-end gap-3">
          <Button onClick={() => navigate("/duels")} className="font-['Barlow_Condensed'] font-bold uppercase tracking-widest text-xs h-10 px-5">
        ...
        <div className="space-y-6">
          ...
          <div className="border border-border bg-card p-5">
            ...
            <div className="space-y-3">
              {[
                { label: "Wins", value: player.wins, icon: <Trophy size={13} /> },
                { label: "Losses", value: player.losses, icon: <Zap size={13} /> },
                { label: "Language", value: player.lang, icon: <Code2 size={13} /> },
                { label: "Win Streak", value: player.streak, icon: <TrendingUp size={13} /> },
              ].map((s) => (
